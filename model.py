...
data = pd.read_csv("train.csv") #importing train data
testData=pd.read_csv("test.csv") #importing test data
data.dropna()
data=data.drop('end_date',axis=1)
data=pd.get_dummies(data)
data= pd.get_dummies(data['year'].astype(str))
print(data)
...
f = pd.melt( data, id_vars='Churn',  value_vars=cols)
g = sns.FacetGrid( f, hue='Churn', col="variable", col_wrap=5, sharex=False, sharey=False )
